...
void Widget::paintGL() { //tu bedziemy bidnowac framebuffer
    ...
    framebuffer3->bind();
        glViewport(0,0, width(), height());
            camera->pos = { 2,1.8,0.8};
            camera->forward = {0.1, -1.2,-1.4};
        render(camera->matrix());
    framebuffer3->unbind();
    ...
    framebuffer4->bind();
        glViewport(0,0, width(), height());
            camera->pos = {0.3,0,-1.7};
            camera->forward = {2.8,0.5,1.5};
        render(camera->matrix());
    framebuffer4->unbind();
    ...
    framebuffer5->bind();
        ...
        render();
    framebuffer5->unbind();
    glViewport(0, 0,width()/4, height()/3.5f);
    fbOutputTexture->bind(0);//te 2 rzeczy sa ze soba powiazane, ta tekstura jest widziana jako sampletext
        fbProgram->use();
        fbProgram->setUniform("SamplerTex", 0);//wybieramy teksutre
        fbOutputMesh->render();
