...
bool GLSLProgram::checkShaderStatus(GLuint shader) {
    ...
    if(stat == GL_FALSE) {
        ...
        qDebug() << "Shader ERROR: " << infoLog << Qt::endl;
        return false;
    }
    return true;
}
...
bool GLSLProgram::checkProgramStatus(GLuint program) {
    ...
    if(stat == GL_FALSE) {
        GLchar infoLog[10240];
        GLint size = -1;
        glGetProgramInfoLog(program, 10240, &size, infoLog);
        qDebug() << "Program ERROR: " << size << Qt::endl;
        qDebug() << "Program ERROR: " << infoLog << Qt::endl;
        return false;
    }
    else
        qDebug() << "Program LINKED"<< Qt::endl;
    return true;
}
...
void GLSLProgram::setUniform(const char* name, mat4 mat) {
    ...
    if(loc != -1)
        ...
        qDebug() << "Uniform variable error: " << name << Qt::endl;
}
